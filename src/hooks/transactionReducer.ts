...
export function transactionReducer(state: TransactionPageState , action:TransactionAction) : TransactionPageState{
    switch(action.type){
        case "fetchedData": 
            return {
                ...state, 
        ...
        case "search": 
            return{...state, 
                ...
                transactions: searchSortFilter(state.category, action.payload, state.sortWord)
            }; 
        case "filter": 
            return {
                ...state, 
                category: action.payload, 
                transactions: searchSortFilter(action.payload, state.searchWord, state.sortWord)
            };
        case "sort": 
            return {
                ...state, 
                sortWord: action.payload, 
                transactions: searchSortFilter(state.category, state.searchWord, action.payload)
            }
        default:
            throw new Error("Invalid action type for transactionReducer");
    }

    function searchSortFilter(category:string, search:string, sort:string):Transaction[]{
        let dataToBeOrdered: Transaction[]=[];  
        if(category === "All Transactions") dataToBeOrdered = [...state.data]; 
            else dataToBeOrdered = state.data.filter(item => item.category.includes(category));
        switch(sort){
            case "Highest": 
